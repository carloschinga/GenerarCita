...
public interface MedicoRepository extends JpaRepository<Medico, String> {
    ...
    @Query(value = "select distinct m.medcod, m.mednam, m.sercod from view_programacion_medico_deta m",
            nativeQuery = true)
    List<Medico> findDistinctMedicosNative();


    @Query(value = "select distinct m.medcod, m.mednam, m.sercod from view_programacion_medico_deta m where m.sercod = :sercod",
            nativeQuery = true)
